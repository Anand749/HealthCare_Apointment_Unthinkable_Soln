...
exports.protect = async (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET || 'secret-token-key-123');
    req.user = await User.findById(decoded.id);
    if (!req.user) {
      return res.status(401).json({ success: false, error: 'User matching token not found' });
    }
    next();
  } catch (err) {
